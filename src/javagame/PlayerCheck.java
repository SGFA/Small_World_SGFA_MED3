...
public class PlayerCheck extends BasicGameState{
...
/*		if (ArrayList play = 1) {
			playerWhite.draw(20,50);
			if (ArrayList play = 1 && player is connected) {
				playerBlack.draw(20,50);	
			}
		}
	
	// check if two players have been reginsted online then the start button will shows up on the screen. 
		if (ArrayList play = 2) {
			playerWhite.draw(20,120);
			if (ArrayList play = 2 && player is connected) {
				playerBlack.draw(20,120);	
				startButton.draw(310,300);
				playNumber = true; 
				}
	...
	*/	
	...
	public void update(GameContainer gc, StateBasedGame sbg, int arg2) throws SlickException {
		...
		// if two players has been connected to the system then press the start key the game will start. 
